...
@section('title', 'Detalhe ' . $cargo->nome)
@section('content')
    ...
    <main class="container" style="min-height: 100vh;">
        <section class="page-inner p-3">
            ...
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    <h3 class="fw-bold mb-1">Cargos</h3>
                    <nav>
                ...
                <div class="d-flex ms-md-autoA py-2 py-md-0 mt-0">
                    <a href="{{ route('cargos.index') }}" class="btn-outline-active rounded-1">
            ...
            <section class="mt-3 rounded-0">
                <div class="card-header card bg-white border-bottom-0 text-activ rounded-top-1 pt-2 pb-2 ">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="mb-0 text-active"><strong>Detalhe do Cargo:</strong></h5>
                ...
                <div class="card card-body rounded-0 bg-white pb-4 pt-4">
                    <strong>Nome:</strong> {{ $cargo->nome }} <br>
                    <strong>Descrição:</strong> {{ $cargo->descricao }} <br>
                    <strong>Departamento:</strong> {{ $cargo->departamento->nome }} <br>
                    <strong>Criado por:</strong> {{ $cargo->user->name }}
                    <strong>Data do Cadastro:</strong> {{ $cargo->created_at->format('d/m/Y') }} <br>
            ...
            <section class="mt-3 rounded-0">
                ...
                <div class="card card-body rounded-0 bg-white">
                    @if ($cargo->colaboradores->isEmpty())
                        <div class="p-5">
                            <div class="text-acitve text-center">
                                <div class="mb-4 ">
                                    <i class="fas fa-exclamation" style="font-size: 80px;"></i>
                                </div>
                                <h5 class="fw-normal mb-3">Não existe colaborador para este Cargo</h5>
                            </div>
                            <div class="text-center">
                                <a href="{{ route('departamento.create') }}" class="btn btn-active rounded-pill">
                                    <i class="ri-user-add-line me-2"></i>Adicionar Colaborador neste Cargo
                                </a>
                            </div>
                        </div>
                    @else
